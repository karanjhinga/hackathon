...
public interface FeatureTeamMappingRepository extends JpaRepository<FeatureTeamMapping, Integer> {
  ...
  @Query(value = "SELECT DISTINCT id, team, features, issue_type, sub_team, SIMILARITY(features, :searchTerm) AS similarity_score " +
  ...
  @Query(value = "SELECT DISTINCT id, team, features, issue_type, sub_team, LEVENSHTEIN(features, :searchTerm) " +
      "FROM feature_team_mapping " +
  ...
  @Query(value = "SELECT DISTINCT id, team, features, issue_type, sub_team, WORD_SIMILARITY(features, :searchTerm) AS word_similarity_score " +
      "FROM feature_team_mapping " +
